        ...
        'products' => [
            'index' => [
                ...
                'create' => [
                    ...
                    'create-btn'              => 'Створити товар',
                    'family'                  => 'Сімейство',
                    'family-hint'             => 'Набір полів для товару. Залиште «За замовчуванням», якщо не впевнені.',
                    'save-btn'                => 'Зберегти товар',
                    'sku'                     => 'Артикул',
                    'sku-placeholder'         => 'наприклад: my-product-001',
                    'title'                   => 'Створити новий товар',
                    'type'                    => 'Тип',
                    'type-hint'               => '*Простий* — один товар, один артикул (наприклад одна футболка). *Конфігурований* — один товар з варіантами: покупець обирає розмір, колір тощо (наприклад футболка S/M/L у червоному/синьому). *Групований* — кілька товарів в одному картці; покупець вказує кількість по кожному. *Бронювання* — події, послуги або оренда (наприклад заняття, номер в готелі).',
                ],
            ...
            'edit' => [
                ...
                'channels' => [
                    'title' => 'Канали',
                    'hint'  => 'Оберіть канали магазину, де цей товар буде відображатися.',
                ],
                ...
                'categories' => [
                    'title' => 'Категорії',
                    'hint'  => 'Категорії — це розділи каталогу: наприклад «Подарунки», «Одяг», «Електроніка», «Кальяни», «Аксесуари». Оберіть одну або кілька категорій, де має показуватися цей товар. Покупці знайдуть його при перегляді цих розділів.',
                ],

                'attribute-hints' => [
                    'sku'                    => 'Артикул (SKU) — унікальний код товару (наприклад JKT-001). Потрібен для обліку, замовлень і пошуку. Має бути унікальним у всьому магазині.',
                    'product_number'        => 'Номер продукту — внутрішній або виробничий номер. Додатковий ідентифікатор до артикулу (наприклад модель або артикул виробника).',
                    'new'                   => 'Показувати мітку «Новинка» на товарі. Зазвичай увімкнуто для нещодавно доданих товарів.',
                    'featured'              => 'Рекомендовані товари можуть показуватися на головній або в окремих блоках.',
                    'visible_individually'  => 'Якщо увімкнено — у товару є власна сторінка за посиланням. Вимикайте для варіантів, які показуються лише всередині конфігурованого товару.',
                    'status'                => 'Увімкнено: товар видно в магазині. Вимкнено: приховано від покупців.',
                    'meta_title'            => 'Заголовок для пошуковиків і вкладки браузера. Коротка фраза з назвою та користю (наприклад: «Синя зимова куртка — чоловіча»).',
                    'meta_description'     => 'Короткий текст, який пошуковики показують під посиланням у видачі. Опішіть товар у 1–2 реченнях, краще 120–160 символів. Не копіюйте повний опис.',
                    'url_key'               => 'Адреса сторінки товару (латиниця, цифри, дефіс). Наприклад: blue-winter-jacket. Можна залишити порожнім — підставиться з назви.',
                    'size'                  => 'Розмір товару (наприклад S, M, L для одягу або 42 для взуття). Використовується для варіантів і фільтрів у магазині.',
                    'color'                 => 'Колір варіанту товару. Показується покупцям для вибору (наприклад Червоний, Синій).',
                    'brand'                 => 'Бренд — виробник або торгова марка товару (наприклад Samsung, Nike). Потрібен для фільтрів у каталозі та довіри покупців.',
                    'price'                 => 'Ціна продажу, яку бачать покупці. Вкажіть основну ціну товару в валюті магазину.',
                    'cost'                  => 'Ваша собівартість або закупівельна ціна. Використовується для звітів по маржі; покупцям не показується.',
                    'special_price'        => 'Акційна (знижена) ціна. Якщо вказана, покупці бачитимуть її замість звичайної в межах вказаного періоду.',
                    'special_price_from'   => 'Дата початку дії спеціальної ціни. Залиште порожнім, якщо знижка без обмеження початку.',
                    'special_price_to'     => 'Дата закінчення дії спеціальної ціни. Залиште порожнім, якщо знижка без обмеження кінця.',
                    'length'               => 'Довжина товару для доставки та відображення. Використовуйте одну одиницю (наприклад см).',
                    'width'                => 'Ширина товару для доставки та відображення. Використовуйте одну одиницю (наприклад см).',
                    'height'               => 'Висота товару для доставки та відображення. Використовуйте одну одиницю (наприклад см).',
                    'weight'               => 'Вага товару для розрахунку доставки. Використовуйте одну одиницю (наприклад кг).',
                    'manage_stock'         => 'Якщо увімкнено, можна вказувати та враховувати кількість по кожному джерелу запасів. Якщо вимкнено, товар вважається завжди в наявності.',
                    'short_description'   => 'Короткий підсумок (1–3 речення) для карток товару, сторінок категорій та пошуку. Допомагає покупцю швидко зрозуміти, що це за товар.',
                    'description'         => 'Повний опис товару на сторінці товару. Може бути довгим, з форматуванням та зображеннями. «Короткий опис» вище — для карток і списків; це поле — для основної сторінки товару.',
                ],

                'meta-group-hint' => 'Ці поля потрібні пошуковикам (Google тощо) і соцмережам: з них формуються заголовок і опис у видачі та при поширенні посилання.',

                'settings-group-hint' => 'Налаштування видимості: чи активний товар, чи показувати як новинку чи рекомендований, чи є окрема сторінка.',

                'price-group-hint' => 'Основна ціна — те, що платить покупець. Спеціальна ціна — тимчасова знижка (вкажіть дати для обмеження періоду). «Ціни для груп покупців» дозволяють встановити різні ціни або знижки по групах (наприклад опт) або від кількості.',
                'shipping-group-hint' => 'Габарити та вага використовуються для розрахунку вартості доставки та відображення. Довжина, ширина, висота в одній одиниці (наприклад см), вага в одній одиниці (наприклад кг).',
                'inventories-group-hint' => '«Очікувана кількість замовлення» — кількість, вже в замовленнях, але ще не відвантажена; вона зарезервована. Вкажіть доступну кількість по кожному джерелу запасів (наприклад основний склад). «Управління запасами» в Налаштуваннях визначає, чи використовується цей блок.',
        ...
        'communications' => [
            'templates' => [
                ...
                'edit' => [
                    ...
                    'send-email-desc'=> 'Надіслати цей шаблон на електронну адресу',
                    'recipient-email'=> 'Email одержувача',
                    'send-btn'       => 'Надіслати зараз',
                ],

                'send' => [
                    'success'          => 'Лист успішно надіслано.',
                    'error'            => 'Не вдалося надіслати лист.',
                    'validation-error' => 'Введіть коректну email адресу.',
                ],
            ...
            'email-broadcast' => [
                ...
                'select-template' => 'Оберіть шаблон (необов’язково)',
                'or-manual' => 'Або введіть тему та текст вручну нижче.',
                'recipients' => 'Отримувачі',
                'subject' => 'Тема',
                'subject-placeholder' => 'Введіть тему листа',
                'message' => 'Повідомлення',
                'message-placeholder' => 'Введіть текст повідомлення тут...',
                'enable-rich-editor' => 'Увімкнути редактор (форматування, зображення)',
    ...
    'components' => [
        ...
        'products' => [
            'search' => [
                ...
                'empty-info'         => 'Введіть запит у поле пошуку вище (назва або артикул). Позначте потрібні товари та натисніть «Додати обраний продукт».',
                ...
                'search-hint'        => 'Знайдіть товари у полі вище, позначте їх і натисніть кнопку додавання.',
