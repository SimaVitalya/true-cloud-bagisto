        ...
        'products' => [
            'index' => [
                ...
                'create' => [
                    ...
                    'create-btn'              => 'Создать продукт',
                    'family'                  => 'Семейство',
                    'family-hint'             => 'Набор полей для товара. Оставьте «По умолчанию», если не уверены.',
                    'save-btn'                => 'Сохранить продукт',
                    'sku'                     => 'Артикул (SKU)',
                    'sku-placeholder'         => 'например: my-product-001',
                    'title'                   => 'Создать новый продукт',
                    'type'                    => 'Тип продукта',
                    'type-hint'               => '*Обычный* — один товар, один артикул (например одна футболка). *Конфигурируемый* — один товар с вариантами: покупатель выбирает размер, цвет и т.д. (например футболка S/M/L в красном/синем). *Групповой* — несколько товаров в одной карточке; покупатель указывает количество по каждому. *Бронирование* — события, услуги или аренда (например занятие, номер в отеле).',
                ],
            ...
            'edit' => [
                ...
                'channels' => [
                    'title' => 'Каналы',
                    'hint'  => 'Выберите каналы магазина, где этот товар будет отображаться.',
                ],
                ...
                'categories' => [
                    'title' => 'Категории',
                    'hint'  => 'Категории — это разделы каталога: например «Подарки», «Одежда», «Электроника», «Аксессуары». Выберите одну или несколько категорий, где должен показываться этот товар. Покупатели увидят его при просмотре этих разделов.',
                ],

                'attribute-hints' => [
                    'sku'                    => 'Артикул (SKU) — уникальный код товара (например JKT-001). Нужен для учёта, заказов и поиска. Должен быть уникальным во всём магазине.',
                    'product_number'        => 'Номер продукта — внутренний или производственный номер. Дополнительный идентификатор к артикулу (например модель или артикул производителя).',
                    'new'                   => 'Показывать метку «Новинка» на товаре. Обычно включают для недавно добавленных товаров.',
                    'featured'              => 'Рекомендованные товары могут показываться на главной или в отдельных блоках.',
                    'visible_individually'  => 'Если включено — у товара есть своя страница и на неё можно перейти по ссылке. Выключайте для вариантов, которые показываются только внутри конфигурируемого товара.',
                    'status'                => 'Включено: товар виден в магазине. Выключено: скрыт от покупателей.',
                    'meta_title'            => 'Заголовок для поисковиков и вкладки браузера. Короткая фраза с названием и пользой (например: «Синяя зимняя куртка — мужская»).',
                    'meta_description'     => 'Краткий текст, который поисковики показывают под ссылкой в выдаче. Опишите товар в 1–2 предложениях, лучше 120–160 символов. Не копируйте полное описание.',
                    'url_key'               => 'Адрес страницы товара (латиница, цифры, дефис). Например: blue-winter-jacket. Можно оставить пустым — подставится из названия.',
                    'size'                  => 'Размер товара (например S, M, L для одежды или 42 для обуви). Используется для вариантов и фильтров в магазине.',
                    'color'                 => 'Цвет варианта товара. Показывается покупателям для выбора (например Красный, Синий).',
                    'brand'                 => 'Бренд — производитель или торговая марка товара (например Samsung, Nike). Нужен для фильтров в каталоге и доверия покупателей.',
                    'price'                 => 'Цена продажи, которую видят покупатели. Укажите основную цену товара в валюте магазина.',
                    'cost'                  => 'Ваша себестоимость или закупочная цена. Используется для отчётов по марже; покупателям не показывается.',
                    'special_price'        => 'Акционная (скидочная) цена. Если указана, покупатели видят её вместо обычной в указанный период.',
                    'special_price_from'   => 'Дата начала действия специальной цены. Оставьте пустым, если скидка без ограничения по началу.',
                    'special_price_to'     => 'Дата окончания действия специальной цены. Оставьте пустым, если скидка без ограничения по концу.',
                    'length'               => 'Длина товара для доставки и отображения. Используйте одну единицу (например см).',
                    'width'                => 'Ширина товара для доставки и отображения. Используйте одну единицу (например см).',
                    'height'               => 'Высота товара для доставки и отображения. Используйте одну единицу (например см).',
                    'weight'               => 'Вес товара для расчёта доставки. Используйте одну единицу (например кг).',
                    'manage_stock'         => 'Если включено, можно указывать и учитывать количество по каждому источнику запасов. Если выключено, товар считается всегда в наличии.',
                    'short_description'   => 'Краткое описание (1–3 предложения) для карточек товара, страниц категорий и поиска. Помогает покупателю быстро понять, что это за товар.',
                    'description'         => 'Полное описание товара на странице товара. Может быть длинным, с форматированием и изображениями. «Короткий опис» выше — для карточек и списков; это поле — для основной страницы товара.',
                ],

                'meta-group-hint' => 'Эти поля нужны поисковикам (Google и др.) и соцсетям: по ним формируется заголовок и описание в выдаче и при расшаривании ссылки.',

                'settings-group-hint' => 'Настройки видимости: активен ли товар, показывать как новинку или рекомендованный, есть ли отдельная страница.',

                'price-group-hint' => 'Основная цена — то, что платит покупатель. Специальная цена — временная скидка (укажите даты для ограничения периода). «Цены для групп покупателей» позволяют задать разные цены или скидки по группам (например опт) или от количества.',
                'shipping-group-hint' => 'Габариты и вес используются для расчёта стоимости доставки и отображения. Длина, ширина, высота в одной единице (например см), вес в одной единице (например кг).',
                'inventories-group-hint' => '«Ожидаемое количество заказа» — количество, уже в заказах, но ещё не отгружено; оно зарезервировано. Укажите доступное количество по каждому источнику запасов (например основной склад). «Управление запасами» в Настройках определяет, используется ли этот блок.',
        ...
        'communications' => [
            'templates' => [
                ...
                'edit' => [
                    ...
                    'send-email-desc'=> 'Отправить этот шаблон на email',
                    'recipient-email'=> 'Email получателя',
                    'send-btn'       => 'Отправить сейчас',
                ],
    ...
    'components' => [
        ...
        'products' => [
            'search' => [
                ...
                'empty-info'         => 'Введите запрос в поле поиска выше (название или артикул). Отметьте нужные товары и нажмите «Добавить выбранный продукт».',
                ...
                'search-hint'        => 'Найдите товары в поле выше, отметьте их и нажмите кнопку добавления.',
